...
;
; PURPOSE:
;       Conversion between any 2 of 4 coord systems for solar images. Slightly modified 
;		version of CNVT_COORD.
;
; EXPLANATION:
...
;               will be used.
;		DOUBlE -- boolean, set to return double precision shifts. 
;
; CALLS:
...
;		Version 5, March 10, 2014, Patrick McCauley, SAO
;		   Added DOUBLE keyword so shifts don't have to be rounded 
;			  before being returned. 
;
; VERSION:
;       Version 5, March 10, 2014
;-
...
FUNCTION MC_CNVT_COORD, xx, yy, csi=csi, from=from, to=to, date=date, $
                     off_limb=off_limb, double=double
   ON_ERROR, 2
...
;---------------------------------------------------------------------------
   ...
   CASE (code) OF
      12: BEGIN
         ix = (csi.ddelt1*(aa(*, 0)-csi.drpix1)+1.0 > 1) ;< csi.naxis1
         iy = (csi.ddelt2*(aa(*, 1)-csi.drpix2)+1.0 > 1) ;< csi.naxis2
         if keyword_set(double) then RETURN, [[ix], [iy]] $
         else RETURN, [[ROUND(ix)], [ROUND(iy)]]
      END
      13: BEGIN
