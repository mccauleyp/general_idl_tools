	PRO MCUTPLOT_IMAGE,IMAGE,XX,YY,NOERASE=NOERASE,	$
		...
		_EXTRA=_EXTRA,CUBIC=CUBIC,CONTOUR=CONTOUR,LCOLOR=LCOLOR, $
                ...
                BSCALED=BSCALED, MEDIAN_STEP=MEDIAN_STEP, TRUE=TRUE, NODATA=NODATA, $
                WHITE_TICKS=WHITE_TICKS
;+
; NOTE: This is a modified version of UTPLOT_IMAGE that adds some features and makes 
; some other small changes (see version history)
;
; Name:
; 
;	UTPLOT_IMAGE
...
;	MEDIAN_STEP = By default, the interpolation of the input image onto a regular grid
;				  (if needed) will use the minimum X & Y step sizes of the input grid. 
;				  Set this keyword to instead use the median step sizes, which is likelier 
;				  to display an image with a similar pixel count as the input. 
...
;	Version 7, 16-Mar-2017, Patrick McCauley
;		Added keyword MEDIAN_STEP
;	Version 7, 28-Apr-2017, Patrick McCauley
;		Added keywords TRUE and NODATA 
;	Version 8, 06-Jun-2017, Patrick McCauley
;		Updated so that the plot axes annotations (titles, tick names) aren't drawn 
;		twice, which can produce an undesired bold appearance in PDF files
;	Version 9, 05-Sep-2017, Patrick McCauley
;		Edited the interpolation so that the full x/y ranges of the input are always displayed, 
;		by sometimes adding another column/row to the grid (use CEIL instead of ROUND). 
;-
;
	ON_ERROR,0
;
...
;
	...
	IF SZ(0) NE 2 THEN BEGIN
		IF KEYWORD_SET(TRUE) THEN BEGIN
			SZ[1:2] = SZ[2:3]
		ENDIF ELSE MESSAGE,'IMAGE must be two-dimensional'
	ENDIF
;
...
;
	...
	NX = N_ELEMENTS(X)
        ...
        IF (X_LINEAR GT 1) OR (Y_LINEAR GT 1) THEN BEGIN  ;we must interpolate
			...
			if keyword_set(median_step) then begin
				scale[0] = median(abs(dx))
				scale[1] = median(abs(dy))			
			endif else begin
			   SCALE[0] = MIN(ABS(DX))
			   SCALE[1] = MIN(ABS(DY))
			endelse

           N_X_INT = ceil((X[NX-1]-X[0])/SCALE[0])
           N_Y_INT = ceil((Y[NY-1]-Y[0])/SCALE[1])
...
;
	...
	if n_elements(_extra) gt 0 then begin
		rem_tags = ['TITLE','XTITLE','YTITLE','XTICKNAME','YTICKNAME']
		_ex1 = rem_tag(_extra, rem_tags)
	endif
	UTPLOT,[X1,X2],[Y1,Y2],BASE_TIME,/NODATA,NOERASE=NOERASE, $
		COLOR=COLOR,CHARSIZE=CHARSIZE, $
		XRANGE=[X1,X2],YRANGE=[Y1,Y2],_EXTRA=_EX1,/XS, $
		xtitle=' ', ytitle=' ', xtickname=strarr(20)+' ', ytickname=strarr(20)+' '
...
;
	...
	IF NOT KEYWORD_SET(NODATA) THEN BEGIN 
		...
		TOP=TOP,BOTTOM=BOTTOM,VELOCITY=VELOCITY,COMBINED=COMBINED, $
                LOWER=LOWER,BSCALED=BSCALED, TRUE=TRUE
...
;
	...
	IF KEYWORD_SET(WHITE_TICKS) THEN BEGIN 
		...
		if n_elements(_extra) gt 0 then begin
			_ex = _extra
			if tag_exist(_ex, 'YTICKLEN') then _ex.yticklen = 0.0001 $
				else _ex = add_tag(_ex, 0.0001, 'yticklen')
			if tag_exist(_ex, 'XTICKLEN') then _ex.xticklen = 0.0001 $
				else _ex = add_tag(_ex, 0.0001, 'xticklen')				
		endif
		
		UTPLOT,[X1,X2],[Y1,Y2],BASE_TIME,/NODATA,/NOERASE,COLOR=255, $
				CHARSIZE=CHARSIZE, XRANGE=[X1,X2],YRANGE=[Y1,Y2], $
				XTICK_GET=XTICK_GET,YTICK_GET=YTICK_GET,$
				xtitle=' ', ytitle=' ', title=' ', $
				xtickname=strarr(20)+' ', ytickname=strarr(20)+' ', $
			_EXTRA=_EX1
