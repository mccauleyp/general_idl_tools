...
; ========================================================================================
;+
; NOTE: This is a slightly modified version of AL_LEGEND that just makes the THICK 
; keyword compatible with USERSYM
;
; NAME:
;       AL_LEGEND
; PURPOSE:
...
;		Made THICK keyword compatible with usersym. February 2015. Patrick McCauley
;       
;-
; ========================================================================================
...
for iclr = 0,clear do begin
 ...
 for i = ii[0],ii[1],ii[2] do begin
  ...
  if (psym[i] eq 8) && (N_elements(usersym) GT 1) then begin 	         
		if size(colors, /tname) eq 'STRING' then $
			usersym,usersym,fill=fill, thick=thick[i]+1 $
		else $
			usersym,usersym,fill=fill,color=colors[i], thick=thick[i]+1
	endif
