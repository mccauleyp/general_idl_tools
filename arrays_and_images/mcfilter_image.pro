function mcfilter_image, image, SMOOTH=width_smooth, ITERATE_SMOOTH=iterate, $
                              MEDIAN=width_median, ALL_PIXELS=all_pixels, $
                              FWHM_GAUSSIAN=fwhm, NO_FT_CONVOL=no_ft, PSF=psf, $
                              XY_CORREL=xy_corr, angle=angle
;+
; NOTE: This is a modified version of the FILTER_IMAGE routine that adds the capability
;       to rotate a PSF by an arbitrary angle before convolution. 
;
; NAME:
;       FILTER_IMAGE
;
...
;
;       ANGLE = float angle to rotate the PSF before convolution
;
...
;		Added XY_CORREL keyword for call to PSF_GAUSSIAN.PRO - P. McCauley 2017
;		Added ANGLE keyword - P. McCauley 2018
;-
  compile_opt idl2
      ...
      return, -1
        ...
        if keyword_set( fwhm ) then begin
                ...
                if N_elements(PSF) EQ 0 then $
                          psf=psf_gaussian( NP=npix,FWHM=fwhm,/NORM, XY_CORREL=xy_corr)
			  	
			  	if n_elements(angle) gt 0 then $
			  		psf = rot(psf, (-1.*angle - 90.), cubic=-0.5)
